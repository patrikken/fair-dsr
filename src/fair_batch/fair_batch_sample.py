# Credit https://github.com/yuji-roh/fairbatch/blob/main/FairBatchSampler.py
import sys
import os
import numpy as np
import math
...
class FairBatch(Sampler):
    ...
    def __init__(self, model, x_tensor, y_tensor, z_tensor, batch_size, alpha, target_fairness, replacement=False, seed=0, T=.001, logger=None):
        ...
        self.x_data = x_tensor.to(self.device)
        self.y_data = y_tensor.to(self.device)
        self.z_data = z_tensor.to(self.device)
        ...
        for tmp_yz in self.yz_tuple:
            self.yz_mask[tmp_yz] = (self.y_data == tmp_yz[0]) & (
                self.z_data == tmp_yz[1])
        ...
        for tmp_yz in self.yz_tuple:
            self.yz_index[tmp_yz] = (
                self.yz_mask[tmp_yz] == 1).nonzero().squeeze()
    ...
    def adjust_lambda(self):
        ...
        if self.fairness_type == 'eqopp':
            ...
            eo_loss = criterion((torch.tanh(logit)+1)/2, (self.y_data+1)/2)

            for tmp_yz in self.yz_tuple:
                yhat_yz[tmp_yz] = float(
                    torch.sum(eo_loss[self.yz_index[tmp_yz]])) / self.yz_len[tmp_yz]

            for tmp_y in self.y_item:
                yhat_y[tmp_y] = float(
                    torch.sum(eo_loss[self.y_index[tmp_y]])) / self.y_len[tmp_y]
            ...
            eo_loss = criterion(
                (torch.tanh(logit)+1)/2, (self.y_data+1)/2)
            ...
            ones_tensor = torch.FloatTensor(ones_array).to(self.device)

            # print(">>>>", logit.shape, ones_tensor.shape)
            # Note that ones tensor puts as the true label
            dp_loss = criterion((torch.tanh(logit)+1)/2, ones_tensor)

            for tmp_yz in self.yz_tuple:
                yhat_yz[tmp_yz] = float(
                    torch.sum(dp_loss[self.yz_index[tmp_yz]])) / self.z_len[tmp_yz[1]]
            ...
            # update only if the disparities exceed the treshold
            if y1_diff > y0_diff:
                if y1_diff > self.T:
                    if yhat_yz[(1, 1)] > yhat_yz[(1, 0)]:
                        self.lb1 += self.alpha
            ...
            if self.logger:
                self.logger.experiment["distpaties(1,1)(1,0)"].log(y1_diff)
                self.logger.experiment["distpaties(-1,1)(-1,0)"].log(y0_diff)
    ...
    def select_batch_replacement(self, batch_size, full_index, batch_num, replacement=False):
        ...
        if replacement == True:
            for _ in range(batch_num):
                select_index.append(np.random.choice(
                    full_index.detach().cpu(), batch_size, replace=False))
        else:
            ...
            for i in range(batch_num):
                if start_idx + batch_size > len(full_index):
                    ...
                    select_index.append(
                        tmp_index[start_idx:start_idx + batch_size])
    ...
    def __iter__(self):
        ...
        if self.fairness_type == 'original':
            ...
            entire_index = torch.FloatTensor(
                [i for i in range(len(self.y_data))])

            sort_index = self.select_batch_replacement(
                self.batch_size, entire_index, self.batch_num, self.replacement)
            ...
            if self.fairness_type == 'eqopp':
                ...
                each_size[(1, 1)] = round(
                    self.lb1 * (self.S[(1, 1)] + self.S[(1, 0)]))
                each_size[(1, 0)] = round((1-self.lb1) *
                                          (self.S[(1, 1)] + self.S[(1, 0)]))
                ...
                each_size[(1, 0)] = round((1-self.lb1) *
                                          (self.S[(1, 1)] + self.S[(1, 0)]))
                each_size[(-1, 1)] = round(self.lb2 *
                                           (self.S[(-1, 1)] + self.S[(-1, 0)]))
                each_size[(-1, 0)] = round((1-self.lb2) *
                                           (self.S[(-1, 1)] + self.S[(-1, 0)]))
            ...
            sort_index_y_1_z_1 = self.select_batch_replacement(
                each_size[(1, 1)], self.yz_index[(1, 1)], self.batch_num, self.replacement)
            sort_index_y_0_z_1 = self.select_batch_replacement(
                each_size[(-1, 1)], self.yz_index[(-1, 1)], self.batch_num, self.replacement)
            sort_index_y_1_z_0 = self.select_batch_replacement(
                each_size[(1, 0)], self.yz_index[(1, 0)], self.batch_num, self.replacement)
            sort_index_y_0_z_0 = self.select_batch_replacement(
                each_size[(-1, 0)], self.yz_index[(-1, 0)], self.batch_num, self.replacement)

            for i in range(self.batch_num):
                key_in_fairbatch = sort_index_y_0_z_0[i].copy()
                key_in_fairbatch = np.hstack(
                    (key_in_fairbatch, sort_index_y_1_z_0[i].copy()))
                key_in_fairbatch = np.hstack(
                    (key_in_fairbatch, sort_index_y_0_z_1[i].copy()))
                key_in_fairbatch = np.hstack(
                    (key_in_fairbatch, sort_index_y_1_z_1[i].copy()))
    ...
    def __len__(self):
        ...
        return len(self.y_data) // self.batch_size
